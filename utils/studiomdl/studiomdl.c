...
{ 
	...
	for (i = 0; i < numseq; i++)
	{
		if (sequence[i].numframes > 1)
		{
			...
			{	
				vec3_t	adj;
				for (k = 0; k < sequence[i].panim->numbones; k++)
				{
					if (sequence[i].panim->node[k].parent == -1)
					{
						ppos = sequence[i].panim->pos[k];

						VectorScale( motion, j * 1.0 / (sequence[i].numframes - 1), adj );
						// printf("  %f %f %f\n", adj[0], adj[1], adj[2] );
						VectorSubtract( sequence[i].panim->pos[k][j], adj, sequence[i].panim->pos[k][j] );
					}
				}
			}
	...
	for (i = 0; i < numseq; i++)
	{
		...
		for (k = 0; k < sequence[i].panim->numbones; k++)
		{
			...
			{
					float	motion[6];
					motion[0] = sequence[i].panim->pos[k][0][0];
					motion[1] = sequence[i].panim->pos[k][0][1];
					motion[2] = sequence[i].panim->pos[k][0][2];
					motion[3] = sequence[i].panim->rot[k][0][0];
					motion[4] = sequence[i].panim->rot[k][0][1];
					motion[5] = sequence[i].panim->rot[k][0][2];
					...
					{	
						...
						*/
						if (type & STUDIO_XR)
							sequence[i].panim->rot[k][j][0] = motion[3];
						if (type & STUDIO_YR)
							sequence[i].panim->rot[k][j][1] = motion[4];
						if (type & STUDIO_ZR)
							sequence[i].panim->rot[k][j][2] = motion[5];
					}
			}
		}
	}
...
void OptimizeAnimations(void)
{
	...
	for (i = 0; i < numseq; i++)
	{
		sequence[i].numframes = sequence[i].panim->endframe - sequence[i].panim->startframe + 1;

		// force looping animations to be looping
		if (sequence[i].flags & STUDIO_LOOPING)
		{
			for (j = 0; j < sequence[i].panim->numbones; j++)
			{
					vec3_t				*ppos = sequence[i].panim->pos[j];
					vec3_t				*prot = sequence[i].panim->rot[j];
		...
		{
			if (sequence[i].event[j].frame < sequence[i].panim->startframe)
			{
				printf( "sequence %s has event (%d) before first frame (%d)\n", sequence[i].name, sequence[i].event[j].frame, sequence[i].panim->startframe );
				sequence[i].event[j].frame = sequence[i].panim->startframe;
				iError++;
			}
			if (sequence[i].event[j].frame > sequence[i].panim->endframe)
			{
				printf( "sequence %s has event (%d) after last frame (%d)\n", sequence[i].name, sequence[i].event[j].frame, sequence[i].panim->endframe );
				sequence[i].event[j].frame = sequence[i].panim->endframe;
				iError++;
			}
		}

		sequence[i].frameoffset = sequence[i].panim->startframe;
		// printf("\n");
	}
	/*
	if (iError)
...
void SimplifyModel (void)
{
	...
	for (i = 0; i < numseq; i++)
	{
		for (j = 0; j < sequence[i].panim->numbones; j++)
		{
			for (k = 0; k < numrenamedbones; k++)
			{
				if (!strcmp( sequence[i].panim->node[j].name, renamedbone[k].from))
				{
					strcpy( sequence[i].panim->node[j].name, renamedbone[k].to );
					break;
				}
			}
		}
	...
	for (i = 0; i < numseq; i++)
	{
		for (k = 0; k < MAXSTUDIOSRCBONES; k++)
		{
			sequence[i].panim->boneimap[k] = -1;
		}
		for (j = 0; j < sequence[i].panim->numbones; j++)
		{
			k = findNode( sequence[i].panim->node[j].name );
			
			if (k == -1)
			{
				// printf("unknown bone \"%s\" in sequence \"%s\"\n", sequence[i].panim[0]->node[j].name, sequence[i].name );
				sequence[i].panim->bonemap[j] = -1;
			}
			else
			{
				char *szAnim = "ROOT";
				...
				if (sequence[i].panim->node[j].parent != -1)
					szAnim = sequence[i].panim->node[sequence[i].panim->node[j].parent].name;
				...
				if (strcmp(szAnim, szNode))
				{
					printf("illegal parent bone replacement in sequence \"%s\"\n\t\"%s\" has \"%s\", reference has \"%s\"\n", 
						sequence[i].name, 
						sequence[i].panim->node[j].name, 
						szAnim,
						szNode );
					iError++;
				}
				sequence[i].panim->bonemap[j] = k;
				sequence[i].panim->boneimap[k] = j;
	...
	for (i = 0; i < numseq; i++)
	{
		...
		vec3_t			*origrot[MAXSTUDIOSRCBONES];
			...
			for (j = 0; j < sequence[i].panim->numbones; j++)
			{
				origpos[j] = sequence[i].panim->pos[j];
				origrot[j] = sequence[i].panim->rot[j];
			}

			for (j = 0; j < numbones; j++)
			{
				if ((k = sequence[i].panim->boneimap[j]) >= 0)
				{
					// link to original animations
					sequence[i].panim->pos[j] = origpos[k];
					sequence[i].panim->rot[j] = origrot[k];
				}
				else
				{
					// link to dummy animations
					sequence[i].panim->pos[j] = defaultpos[j];
					sequence[i].panim->rot[j] = defaultrot[j];
				}
			}
		...
		for (k = 0; k < 6; k++)
		{
			...
			for (i = 0; i < numseq; i++)
			{
					for (n = 0; n < sequence[i].numframes; n++)
					{
						...
						case 1: 
						case 2: 
							v = ( sequence[i].panim->pos[j][n][k] - bonetable[j].pos[k] ); 
							break;
						case 3:
						case 4:
						case 5:
							v = ( sequence[i].panim->rot[j][n][k-3] - bonetable[j].rot[k-3] ); 
							if (v >= Q_PI)
								v -= Q_PI * 2;
							if (v < -Q_PI)
								v += Q_PI * 2;
		...
		for (j = 0; j < 3; j++)
		{
			...
			for (n = 0; n < sequence[i].numframes; n++)
			{
				...
				for (j = 0; j < numbones; j++)
				{
					...
					// convert to degrees
					angle[0]	= sequence[i].panim->rot[j][n][0] * (180.0 / Q_PI);
					angle[1]	= sequence[i].panim->rot[j][n][1] * (180.0 / Q_PI);
					angle[2]	= sequence[i].panim->rot[j][n][2] * (180.0 / Q_PI);

					AngleMatrix( angle, bonematrix );

					bonematrix[0][3] = sequence[i].panim->pos[j][n][0];
					bonematrix[1][3] = sequence[i].panim->pos[j][n][1];
					bonematrix[2][3] = sequence[i].panim->pos[j][n][2];
				...
				for (j = 0; j < numbones; j++)
				{
					for (k = 0; k < 6; k++)
					{
						...
						for (n = 0; n < sequence[i].numframes; n++)
						{
							...
							case 1: 
							case 2: 
								value[n] = ( sequence[i].panim->pos[j][n][k] - bonetable[j].pos[k] ) / bonetable[j].posscale[k]; 
								break;
							case 3:
							case 4:
							case 5:
						...
						sequence[i].panim->numanim[j][k] = 0;
						...
						sequence[i].panim->numanim[j][k] = pvalue - data;
						if (sequence[i].panim->numanim[j][k] == 2 && value[0] == 0)
						{
							sequence[i].panim->numanim[j][k] = 0;
						}
						else
						{
							sequence[i].panim->anim[j][k] = kalloc( pvalue - data, sizeof( mstudioanimvalue_t ) );
							memmove( sequence[i].panim->anim[j][k], data, (pvalue - data) * sizeof( mstudioanimvalue_t ) );
						}
		...
		for (i = 0; i < numseq; i++)
		{
			...
			if (sequence[i].activity == 0)
			{
					for (j = 0; j < numbones; j++)
					{
						for (k = 0; k < 6; k++)
						{
							accum += sequence[i].panim->numanim[j][k] * sizeof( mstudioanimvalue_t );
						}
					}
...
int Cmd_Sequence( )
{
	...
	for (i = 0; i < numblends; i++)
	{
		...
		sequence[numseq].panim = panimation[numani];
		sequence[numseq].panim->startframe = start;
		sequence[numseq].panim->endframe = end;
		sequence[numseq].panim->flags = 0;
